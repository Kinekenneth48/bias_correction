...
source(file = "R/gbm_lnorm_test.R")
...
min_MSE <- which.min(gbm_fit$cv.error)


# get MSE and compute RMSE
sqrt(gbm_fit$cv.error[min_MSE])
...
# predict values
df_train$pred_gbm <- predict(gbm_fit, n.trees = gbm_fit$n.trees, df_train)
...
plot(df_train$RATIO, df_train$pred_gbm, xlim = c(0, 1), ylim = c(0, 1))
abline(a = 0, b = 1, col = "blue")


descdist(df_train$error_gbm, discrete = FALSE, boot = 1000)

fit_norm <- fitdist(df_train$error_gbm, "norm")
plot(fit_norm)
...
# 
# # predict values
# df_test$pred_gbm <- predict(gbm_fit, n.trees = gbm_fit$n.trees, df_test)
# 
# df_test = df_test %>%
#   mutate(swe_gbm = pred_gbm*maxv_SNWD)
# 
# 
# 
# df_test_filter <- df_test %>%
...
# 
# 
# lnorm_test_actual = do.call(rbind,lapply(X = ls_df_test_lorm,
#                           FUN = fit_lnorm, column = "maxv_WESD" ))
# 
# 
# lnorm_test_converted_gbm = do.call(rbind, lapply(X = ls_df_test_lorm,
#                           FUN = fit_lnorm, column = "swe_gbm" ))
# 
# 
# save(lnorm_test_actual, file = "data-raw/RObject/lnorm_test_actual.RData")
# save(lnorm_test_converted_gbm, file = "data-raw/RObject/lnorm_test_converted_gbm.RData")
# 
...
# specify percentiles
percentile <- c(0.38, 0.85, 0.9, 0.92, 0.94, 0.96, 0.98, 1)
...
for (i in percentile) {
  ...
  assign(paste0("converted_load_para_q_", i, "_lnorm_gbm"), df)
  
  file_name <- paste0("converted_load_para_q_", i, "_lnorm_gbm")
  
  # save downloaded data
  save(
    list = paste0("converted_load_para_q_", i, "_lnorm_gbm"),
    file = paste0("data-raw/RObject/", file_name, ".RData")
  )
  
  # remove data
  remove(list = paste0("converted_load_para_q_", i, "_lnorm_gbm"))
  rm(df)
}
...
load("data-raw/RObject/converted_load_para_q_85_lnorm_gbm.RData")
load("data-raw/RObject/converted_load_para_q_90_lnorm_gbm.RData")
load("data-raw/RObject/converted_load_para_q_92_lnorm_gbm.RData")
load("data-raw/RObject/converted_load_para_q_94_lnorm_gbm.RData")
load("data-raw/RObject/converted_load_para_q_96_lnorm_gbm.RData")
load("data-raw/RObject/converted_load_para_q_98_lnorm_gbm.RData")
load("data-raw/RObject/converted_load_para_q_100_lnorm_gbm.RData")
load("data-raw/RObject/direct_load_para_lnorm.RData")



# append station id
ID <- rownames(converted_load_para_q_38_lnorm_gbm)
converted_load_para_q_38_lnorm_gbm <- cbind(ID, converted_load_para_q_38_lnorm_gbm)

ID <- rownames(converted_load_para_q_85_lnorm_gbm)
converted_load_para_q_85_lnorm_gbm <- cbind(ID, converted_load_para_q_85_lnorm_gbm)

ID <- rownames(converted_load_para_q_90_lnorm_gbm)
converted_load_para_q_90_lnorm_gbm <- cbind(ID, converted_load_para_q_90_lnorm_gbm)

ID <- rownames(converted_load_para_q_92_lnorm_gbm)
converted_load_para_q_92_lnorm_gbm <- cbind(ID, converted_load_para_q_92_lnorm_gbm)

ID <- rownames(converted_load_para_q_94_lnorm_gbm)
converted_load_para_q_94_lnorm_gbm <- cbind(ID, converted_load_para_q_94_lnorm_gbm)

ID <- rownames(converted_load_para_q_96_lnorm_gbm)
converted_load_para_q_96_lnorm_gbm <- cbind(ID, converted_load_para_q_96_lnorm_gbm)

ID <- rownames(converted_load_para_q_98_lnorm_gbm)
converted_load_para_q_98_lnorm_gbm <- cbind(ID, converted_load_para_q_98_lnorm_gbm)

ID <- rownames(converted_load_para_q_100_lnorm_gbm)
converted_load_para_q_100_lnorm_gbm <- cbind(ID, converted_load_para_q_100_lnorm_gbm)
...
converted_load_para_q_38_lnorm_gbm <- as.data.frame(converted_load_para_q_38_lnorm_gbm) %>%
  mutate(
    loc_38_const = exp(as.numeric(meanlog)),
    sd_38_const = exp(as.numeric(sdlog))
  ) %>%
  dplyr::select(ID, loc_38_const, sd_38_const)

converted_load_para_q_85_lnorm_gbm <- as.data.frame(converted_load_para_q_85_lnorm_gbm) %>%
  mutate(
...
converted_load_para_q_90_lnorm_gbm <- as.data.frame(converted_load_para_q_90_lnorm_gbm) %>%
  mutate(
...
converted_load_para_q_92_lnorm_gbm <- as.data.frame(converted_load_para_q_92_lnorm_gbm) %>%
  mutate(
...
converted_load_para_q_94_lnorm_gbm <- as.data.frame(converted_load_para_q_94_lnorm_gbm) %>%
  mutate(
...
converted_load_para_q_96_lnorm_gbm <- as.data.frame(converted_load_para_q_96_lnorm_gbm) %>%
  mutate(
...
converted_load_para_q_98_lnorm_gbm <- as.data.frame(converted_load_para_q_98_lnorm_gbm) %>%
  mutate(
...
converted_load_para_q_100_lnorm_gbm <- as.data.frame(converted_load_para_q_100_lnorm_gbm) %>%
  mutate(
...
comb_para_bean_lnorm <- inner_join(
  x = direct_load_para_lnorm, y = converted_load_para_q_100_lnorm_gbm,
  by = c("ID")
)

comb_para_bean_lnorm <- inner_join(
  x = comb_para_bean_lnorm, y = converted_load_para_q_98_lnorm_gbm,
  by = c("ID")
)

comb_para_bean_lnorm <- inner_join(
  x = comb_para_bean_lnorm, y = converted_load_para_q_96_lnorm_gbm,
  by = c("ID")
)

comb_para_bean_lnorm <- inner_join(
  x = comb_para_bean_lnorm, y = converted_load_para_q_94_lnorm_gbm,
  by = c("ID")
)


comb_para_bean_lnorm <- inner_join(
  x = comb_para_bean_lnorm, y = converted_load_para_q_92_lnorm_gbm,
  by = c("ID")
)


comb_para_bean_lnorm <- inner_join(
  x = comb_para_bean_lnorm, y = converted_load_para_q_90_lnorm_gbm,
  by = c("ID")
)

comb_para_bean_lnorm <- inner_join(
  x = comb_para_bean_lnorm, y = converted_load_para_q_85_lnorm_gbm,
  by = c("ID")
)

comb_para_bean_lnorm <- inner_join(
  x = comb_para_bean_lnorm, y = converted_load_para_q_38_lnorm_gbm,
  by = c("ID")
)
...
comb_para_bean_lnorm <- comb_para_bean_lnorm %>%
  mutate(
    ...
    ratio_loc_38_const = loc_38_const / loc,
    ratio_sd_38_const = sd_38_const / sd
  )
...
data_long <- comb_para_bean_lnorm %>% pivot_longer(
  cols = c(
    ...
    "ratio_loc_85_const", "ratio_sd_85_const", "ratio_loc_38_const",
    "ratio_sd_38_const"
  ),
...
data_long <- data_long %>%
  mutate(method = case_when(
    ...
    variable == "ratio_loc_38_const" ~ "38p of parameters",
    variable == "ratio_sd_38_const" ~ "38p of parameters"
  ))
...
data_long <- data_long %>%
  mutate(variable = case_when(
    ...
    variable == "ratio_loc_38_const" ~ "mean",
