...
source(file = "R/rf_sd_percentile.R")
...
# specify percentiles
percentile <- c(0.76, 0.8, 0.9, 0.92, 0.94, 0.96, 0.98, 1)
...
load("data-raw/RObject/converted_load_para_q_76_lnorm_rf_test.RData")
...
ID <- rownames(converted_load_para_q_76_lnorm_rf_test)
converted_load_para_q_76_lnorm_rf_test <- cbind(ID, converted_load_para_q_76_lnorm_rf_test)
...
converted_load_para_q_76_lnorm_rf_test <- as.data.frame(converted_load_para_q_76_lnorm_rf_test) %>%
  mutate(
    loc_76_const = exp(as.numeric(meanlog)),
    sd_76_const = exp(as.numeric(sdlog))
  ) %>%
  dplyr::select(ID, loc_76_const, sd_76_const)
...
comb_para_bean_lnorm <- inner_join(
  x = comb_para_bean_lnorm, y = converted_load_para_q_76_lnorm_rf_test,
  by = c("ID")
)
...
comb_para_bean_lnorm <- comb_para_bean_lnorm %>%
  mutate(
    ...
    ratio_loc_76_const = loc_76_const / loc,
    ratio_sd_76_const = sd_76_const / sd,
...
data_long <- comb_para_bean_lnorm %>% pivot_longer(
  cols = c(
    ...
    "ratio_loc_76_const", "ratio_sd_76_const", "ratio_loc_80_const",
    "ratio_sd_80_const"
  ),
...
data_long <- data_long %>%
  mutate(method = case_when(
    ...
    variable == "ratio_loc_76_const" ~ "76p of parameters",
    variable == "ratio_sd_76_const" ~ "76p of parameters",
...
data_long <- data_long %>%
  mutate(variable = case_when(
    ...
    variable == "ratio_loc_76_const" ~ "mean",
...
data_long$method <- factor(data_long$method,
  levels = c(
    ...
    "80p of parameters", "76p of parameters"
  )
)
...
ggplot(data_long, aes(x = variable, y = value, fill = variable)) +
  ...
  ylim(c(0.6, 1.4)) +
  ggtitle("Boxplot comparing the relative parameter ratio of direct load vs converted load with different paramter percentile(GBM- replace = TRUE)")
